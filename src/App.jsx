...
function ClassroomPage({ data, onBack }) {
  ...
  const [fileResourceDraft, setFileResourceDraft] = useState({ title: '', type: 'PDF', description: '', fileName: '', fileMime: '', dataUrl: '' })
  const [activityDraft, setActivityDraft] = useState({ title: '', instructions: '', dueDate: '' })
  ...
  const handleFileResourceChange = (event) => {
    const file = event.target.files?.[0]
    if (!file) return
    const reader = new FileReader()
    reader.onload = () => setFileResourceDraft((prev) => ({ ...prev, fileName: file.name, fileMime: file.type, dataUrl: String(reader.result || ''), type: file.type.includes('pdf') ? 'PDF' : file.type.startsWith('image/') ? 'Image' : 'File' }))
    reader.readAsDataURL(file)
  }
  const handleSaveFileResource = (event) => {
    event.preventDefault()
    if (!fileResourceDraft.title.trim() || !fileResourceDraft.dataUrl) return
    const payload = { id: Date.now(), title: fileResourceDraft.title.trim(), type: fileResourceDraft.type, description: fileResourceDraft.description.trim(), fileName: fileResourceDraft.fileName, fileMime: fileResourceDraft.fileMime, dataUrl: fileResourceDraft.dataUrl, source: 'upload' }
    notify({ ...classData, resources: [payload, ...(classData.resources || [])] }, `New resource added: ${payload.title}.`)
    setFileResourceDraft({ title: '', type: 'PDF', description: '', fileName: '', fileMime: '', dataUrl: '' })
  }
  const handleSaveActivityResource = (event) => {
    event.preventDefault()
    if (!activityDraft.title.trim() || !activityDraft.instructions.trim()) return
    const payload = { id: Date.now(), title: activityDraft.title.trim(), type: 'Activity', description: activityDraft.instructions.trim(), dueDate: activityDraft.dueDate, source: 'activity' }
    notify({ ...classData, resources: [payload, ...(classData.resources || [])] }, `New resource added: ${payload.title}.`)
    setActivityDraft({ title: '', instructions: '', dueDate: '' })
  }
    ...
    <main className="classroom-screen">
      ...
      <section className="container classroom-shell">
        ...
        {activeTab === 'Classroom' ? (
          ...
          <section className="classroom-overview-grid">
            <article className="classroom-content classroom-students"><h2>Class Resources</h2><div className="story-list">{(classData.resources || []).map((resource) => <article className="classroom-student-card" key={resource.id}><span>{resource.type}</span><p><strong>{resource.title}</strong></p><p>{resource.description || 'No description yet.'}</p>{resource.link && (/youtu\.be|youtube\.com/.test(resource.link) ? <iframe className="resource-video" src={resource.link.replace('watch?v=', 'embed/')} title={resource.title} allowFullScreen /> : <div className="resource-link-card"><p>{resource.link}</p><a className="btn btn-outline" href={resource.link} target="_blank" rel="noreferrer">Open Link</a></div>)}{resource.dataUrl && resource.fileMime?.startsWith('image/') && <img className="resource-image" src={resource.dataUrl} alt={resource.fileName || resource.title} />}{resource.dataUrl && resource.fileMime?.includes('pdf') && <div className="resource-link-card"><p>PDF uploaded: {resource.fileName}</p><a className="btn btn-outline" href={resource.dataUrl} download={resource.fileName || 'resource.pdf'}>Open / Download</a></div>}{resource.dataUrl && !resource.fileMime?.startsWith('image/') && !resource.fileMime?.includes('pdf') && <div className="resource-link-card"><p>{resource.fileName} ({resource.fileMime || 'file'})</p><a className="btn btn-outline" href={resource.dataUrl} download={resource.fileName || 'resource'}>Download File</a></div>}{resource.dueDate && <p><strong>Due:</strong> {resource.dueDate}</p>}{data.from === 'teacher' && <div className="teacher-class-actions"><button className="btn btn-outline" type="button" onClick={() => handleDeleteResource(resource.id)}>Delete</button></div>}</article>)}{(classData.resources || []).length === 0 && <p>No resources yet.</p>}</div></article>
            {data.from === 'teacher' && <article className="classroom-content"><h2>Add Link Resource</h2><form className="classroom-add-form" onSubmit={handleSaveResource}><input value={resourceDraft.title} onChange={(e) => setResourceDraft((prev) => ({ ...prev, title: e.target.value }))} placeholder="Resource title" /><select value={resourceDraft.type} onChange={(e) => setResourceDraft((prev) => ({ ...prev, type: e.target.value }))}><option>PDF</option><option>Video</option><option>Quiz</option><option>Worksheet</option><option>Link</option><option>Activity</option></select><textarea value={resourceDraft.description} onChange={(e) => setResourceDraft((prev) => ({ ...prev, description: e.target.value }))} placeholder="Short description" rows={3} /><input value={resourceDraft.link} onChange={(e) => setResourceDraft((prev) => ({ ...prev, link: e.target.value }))} placeholder="https://resource-link.com" /><button className="btn btn-role-continue" type="submit">Save Resource</button></form><hr /><h2>Upload Local File Resource</h2><p>Demo upload saved locally in this browser.</p><form className="classroom-add-form" onSubmit={handleSaveFileResource}><input value={fileResourceDraft.title} onChange={(e) => setFileResourceDraft((prev) => ({ ...prev, title: e.target.value }))} placeholder="File resource title" /><textarea value={fileResourceDraft.description} onChange={(e) => setFileResourceDraft((prev) => ({ ...prev, description: e.target.value }))} placeholder="Short description" rows={2} /><input type="file" accept=".pdf,.png,.jpg,.jpeg,.gif,.doc,.docx,.ppt,.pptx" onChange={handleFileResourceChange} /><button className="btn btn-role-continue" type="submit">Save File Resource</button></form><hr /><h2>Create Activity Resource</h2><form className="classroom-add-form" onSubmit={handleSaveActivityResource}><input value={activityDraft.title} onChange={(e) => setActivityDraft((prev) => ({ ...prev, title: e.target.value }))} placeholder="Activity title" /><textarea value={activityDraft.instructions} onChange={(e) => setActivityDraft((prev) => ({ ...prev, instructions: e.target.value }))} placeholder="Instructions" rows={3} /><input type="date" value={activityDraft.dueDate} onChange={(e) => setActivityDraft((prev) => ({ ...prev, dueDate: e.target.value }))} /><button className="btn btn-role-continue" type="submit">Save Activity</button></form></article>}
          </section>
        ) : activeTab === 'Chat' ? (
          <section className="classroom-content classroom-chat"><header className="classroom-chat-header"><h2>{classData.name}</h2><p>Class group chat</p></header><div className="classroom-chat-messages">{(classData.chatMessages || []).map((message) => <article key={message.id} className={`classroom-chat-bubble ${message.sender === 'Teacher' ? 'is-teacher' : 'is-student'}`}><p><strong>{message.sender}</strong> · {new Date(message.createdAt).toLocaleTimeString()}</p><p>{message.text}</p></article>)}{(classData.chatMessages || []).length === 0 && <p className="classroom-chat-empty">No messages yet. Start the class conversation.</p>}</div><form className="classroom-chat-form" onSubmit={handleSendChat}><input value={chatMessage} onChange={(event) => setChatMessage(event.target.value)} placeholder="Write a message to the class..." /><button className="btn btn-role-continue" type="submit">Send</button></form></section>
        ) : (
